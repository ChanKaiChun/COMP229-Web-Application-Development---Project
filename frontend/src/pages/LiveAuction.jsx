...
const LiveAuction = () => {
    ...
    useEffect(() => {
        axios.get("/api/auction/live")
            .then(response => {
                console.log("Live auctions data:", response.data);
            ...
            .catch(error => {
                console.error("There was an error fetching the live auctions!", error);
            });
    }, []);
                    ...
                    <div key={auction.id} className="border rounded-lg overflow-hidden shadow-lg bg-black flex">
                        <div className="p-4 flex-1 flex flex-col justify-between">
                            <div>
                                ...
                                <div className={`mt-2 text-sm text-white ${getBackgroundColor(timeLefts[index])} p-2 rounded`}>
                                    Time Left: {formatTime(timeLefts[index])}
                                </div>
